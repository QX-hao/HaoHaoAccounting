...
`ContentType` enforces declared request media types on routes with bodies, including structured `application/*+json` variants for JSON endpoints and multipart form data for import endpoints. Configured media types are normalized, deduplicated, and invalid values are ignored before request checks run.
